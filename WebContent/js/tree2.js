...
dTree.prototype.node = function (node, nodeId) {
	...
	if (this.config.useIcons) {
		...
		// 此处修改了树根的显示图片，根据nodeId=0可以判断是树根。（2012-04-13 19:07:52）
		if (0 == nodeId) {
			str += "<img id=\"i" + this.obj + nodeId + "\" src=\"images/tree/folder_header.gif\" alt=\"树根\" />";
		} else {
	...
	if (node.url) {
		...
		if ((!this.config.folderLinks || !node.url) && node._hc && node.pid != this.root.id) {
			str += "<a id=\"s" + this.obj + nodeId + "\" href=\"javascript: " + this.obj + ".o(" + nodeId + ");\" onclick=\"javascript:" + this.obj + ".s(" + nodeId + ");if(window.nodeOnClick){ nodeOnClick(event,'" + node.id + "','" + node.name + "'});\" class=\"node\">";
		} else {
			str += "<a id=\"s" + this.obj + nodeId + "\" href=\"#\" onclick=\"javascript: " + this.obj + ".s(" + nodeId + ");if(window.nodeOnClick){nodeOnClick(event,'" + node.id + "','" + node.name + "')};\" class=\"node\">";
		}
	}
	str += node.name;
